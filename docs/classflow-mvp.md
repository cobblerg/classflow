...
# PART 11. STEP 18 수강생 명단 관리 명세 및 개인정보 주의

## 1. 명단 준비 방식
- **방법 A (자동 생성)**: `studentCount`에 따라 번호순 placeholder(`01 수강생01`, `01 학생01`)를 자동 생성
- **방법 B (CSV 업로드)**: `.csv` 파일(UTF-8)을 파싱하여 실명 명단(`01 김민지`, `02 이준호`)으로 강의 생성 (최대 40명)

## 2. 수강생 개별 이름 수정
- 강사 대시보드 내 [강의 설정]의 **수강생/학생 관리** 영역에서 인라인으로 이름 수정 지원
- **핵심 원칙**: `student.id`와 `student.number`는 절대 불변 유지되며, 기존 진행도(Progress), 이해도(Understanding), 도움 요청(HelpRequest), 피드백(Feedback) 데이터는 100% 보존됨
